...
appcenter test run uitest --app $appName --devices $deviceSetName --app-path $APPCENTER_OUTPUT_DIRECTORY/UiTest.ipa --test-series $testSeriesName --locale "en_US" --build-dir $APPCENTER_SOURCE_DIRECTORY/UiTest/bin/Debug --uitest-tools-dir $APPCENTER_SOURCE_DIRECTORY/packages/XCUITest.$(build.sourcesdirectory)/tools --token $appCenterLoginApiToken
